 ...
 * Generates outfits for each day in a WeatherWeek while avoiding
 * back-to-back duplicates.
 */
public class WeeklyPlannerInteractor {
    /**
     * wardrobeRepository: The actual wardrobe to draw from.
     */
    private final WardrobeRepository wardrobeRepository;
    /**
     * outfitCreator: The creator to handle logic.
     */
    private final OutfitCreator outfitCreator;

    /**
     * Creates an instance of it.
     * @param wardrobeRepository The specific wardrobe to select from.
     * @param outfitCreator generic outfitCreator.
     */
    public WeeklyPlannerInteractor(WardrobeRepository wardrobeRepository,
     ...
     * @param week a WeatherWeek, specifically for this location.
     */
    public void generateOutfitsForWeek(final WeatherWeek week) {
        if (week == null) {
            return;
        }

        Map<String, List<ClothingArticle>> wardrobeMap =
                buildWardrobeMap(wardrobeRepository.getAll());
    ...
    private boolean isWardrobeEmpty(final Map<String,
            List<ClothingArticle>> wardrobeMap) {
    ...
    private Outfit generateOutfitWithNoRepeat(final WeatherDay day,
        ...
        if (!isSameTopOrBottom(first, previousNames) && !isSameOutfit(first,
                previousOutfit)) {
            return first;
        }

        for (int attempt = 0; attempt < 4; attempt++) {
            Outfit candidate = outfitCreator.createOutfitForDay(day,
                    shuffledCopy(wardrobeMap), false);
            if (candidate != null
                    && !isSameTopOrBottom(candidate, previousNames)
        ...
        for (Map.Entry<String,
                List<ClothingArticle>> entry : wardrobeMap.entrySet()) {
            List<ClothingArticle> list = new ArrayList<>();
            for (ClothingArticle item : entry.getValue()) {
                if (item != null && (previousNames == null
                        || !previousNames.contains(item.getName()))) {
                    list.add(item);
                }
            }
    ...
    private Map<String, List<ClothingArticle>>
    shuffledCopy(final Map<String, List<ClothingArticle>> original) {
        Map<String, List<ClothingArticle>> copy = new HashMap<>();
        for (Map.Entry<String,
                List<ClothingArticle>> entry : original.entrySet()) {
    ...
    private boolean isSameTopOrBottom(final Outfit outfit,
        ...
        boolean sameTop = top != null && top.getName()
                != null && previousNames.contains(top.getName());
        boolean sameBottom = bottom != null && bottom.getName()
                != null && previousNames.contains(bottom.getName());
     ...
     * @param a first outfit
     * @param b second outfit
     * @return T/F is the same outfit
     */
    public boolean isSameOutfit(final Outfit a, final Outfit b) {
        if (a != null && b != null) {
    ...
    private Map<String, List<ClothingArticle>>
    buildWardrobeMap(final List<ClothingArticle> items) {
